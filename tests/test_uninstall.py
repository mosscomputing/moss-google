...
_UNINSTALL_PATH = Path(__file__).resolve().parent.parent / "moss_google" / "uninstall.py"
_spec = importlib.util.spec_from_file_location("moss_google_uninstall", _UNINSTALL_PATH)
...
def _run(monkeypatch, args):
    monkeypatch.setattr(sys, "argv", ["moss_google.uninstall", *args])
...
def test_manifest_stays_valid_only_moss_removed(tmp_path, monkeypatch):
    ...
    if tomllib is not None:
        ...
        assert any("google" in d for d in deps)
